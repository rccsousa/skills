...
Dispatch `surgical-review` (or `superpowers:requesting-code-review`). The reviewer MUST **trace each finding and self-classify severity** (RED/ORANGE/YELLOW or Critical/Major/Minor) — first-pass severities are unreliable (see `verify-review-findings.md`). Then apply `receiving-code-review`: verify each finding against actual code before acting.
