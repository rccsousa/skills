...
description: >
  ...
  Work too big for one session or still wrapped in fog hands to `/wayfinder`
  first (multi-session planning front-end), then each resolved slice enters this
  pipeline.
...
| simplify             | **yes**           | `/simplify` fans out 4 cleanup agents itself (reuse / simplify / efficiency / altitude)   |
| verify               | no                | serial runtime observation — drive the app once, capture evidence                         |
...
brief
  ...
  │
  └─ plan ──(council)── implement ── verify¹ ── review ── fix ── simplify ── verify² ── ADR write ── retire scratch ── push + create-PR ── ready-to-merge
```

`verify¹` = fail-fast runtime check right after implement (does the change
...
Before the scope check, read `docs/agents/one-shot.md` (written by
`/setup-skills`) for repo-specific locations. Fall back to
...
- **Fits one session** (the way to the destination is already visible — you can
  hold the whole shape in one ~100K-token context) → run the pipeline below.
- **Spans many sessions / foggy** (bigger than one session can hold, or the
  destination itself isn't clear yet — "loose idea", "figure out how we'd…",
  "explore whether…") → **wayfinder mode** (step 1b). Chart the route first,
  THEN each resolved slice enters the pipeline.

**Then, for single-session work, pick pipeline shape:**
...
### 1b. Wayfinder mode — multi-session efforts

When the work is too big for one session or wrapped in fog (destination not yet
visible), don't charge at the pipeline — **chart the way first** via
`/wayfinder`. Wayfinder is the planning front-end; one-shot is the execution
back-end. They compose:

```
loose idea
   │
   ▼
/wayfinder  ── chart map (destination + fog) ──► resolve tickets one-per-session
   │            (research / prototype / grilling / task — decisions, not deliverables)
   │
   ▼  way is clear — map exhausted, route known
per-slice hand-off
   │
   ▼
one-shot pipeline  (plan → implement → review → fix)  per resolved slice
```

- **Charting** and **each ticket resolution** are separate sessions — wayfinder's
  hard rule: never resolve more than one ticket per session. one-shot does **not**
  drive that loop unattended; it hands to `/wayfinder` and stops.
- The map lives on the issue tracker (`wayfinder:map` label), or the
  local-markdown tracker fallback if no tracker is configured (`/setup-skills`).
- **Skilling ↔ looping.** Wayfinder resolves the *unknowns* (what should this be?);
  one-shot ships the *knowns* (build the thing we now know we want). A resolved
  wayfinder slice whose destination was "a spec to hand off" becomes the PRD/plan
  that seeds one-shot's step 2. The two are the loop: skill the fog into
  decisions, loop the decisions into merged code.
- **auto mode never enters wayfinder** — charting + ticket resolution are HITL by
  construction (grilling/prototype tickets need the human's live side). If auto is
  requested on a foggy brief → halt, tell the user to wayfinder it first.
...
- Multi-PR: `/write-a-prd` (personal skill).
...
Cascade continues into the early verify gate.
...
Rationale for early: catching "doesn't run at all" here is far cheaper than
discovering it after a full review + fix + simplify + ADR pass.

### 4. Review phase — local diff

Dispatch a review agent that invokes `/code-review` (companion skill).
...
Dispatch a fix agent that invokes `/code-fix --findings=<packet>` (companion skill).
...
### 6b. Simplify phase — quality cleanup

Correctness is settled (review + fix landed). Now clean up quality. Dispatch a
simplify agent that invokes `/simplify` (built-in skill) on the diff.

`/simplify` fans out 4 cleanup agents in parallel (reuse / simplification /
efficiency / altitude) and applies behavior-preserving fixes. It is **quality
only — it does not hunt bugs** (that was step 4). It complements `/code-review`,
it does not repeat it.

**Gate (surface before apply):**

- `hitl` / `grill`: `/simplify` surfaces its findings first; user approves the
  set before any edit lands. Then it applies + commits via `/create-commit`.
- `auto`: applies unattended; skipped findings (behavior-changing, out-of-diff,
  false-positive) are noted in the return payload, not argued.

Simplify commits are separate from fix commits so history stays legible
(`refactor:` / `perf:` vs `fix:`). Skip entirely if the diff is trivial or
already clean — `/simplify` will say so.
...
- **SKIP** on no-runtime-surface diffs (same rule as 3b).
- `hitl` / `grill`: evidence surfaced; user reads before ADR/push.
- `auto`: **authoritative gate.** Broken runtime here **HALTS** the cascade
  before ADR write / push / PR open. A change that passed early verify but
  fails now means fix or simplify regressed it — surface the delta.

If early verify SKIPped (no runtime surface), final verify SKIPs too — nothing
to observe. If the diff *gained* a runtime surface between 3b and 6c (rare),
final verify runs even though early SKIPped.
...
as the verification (those already ran); they prove CI passes, not that the
change works. Drive the real surface (CLI / API / GUI / library export).

If the diff has no runtime surface (docs / tests / type-decls / build-config
only), report `SKIP — no runtime surface: <reason>` and return.

Return: PASS (with the captured evidence — command + observed output/screenshot)
| FAIL (evidence of the broken flow) | SKIP (reason).
```

In `auto` mode a FAIL return is a HARD HALT — do not proceed to the next phase,
surface the evidence. In `hitl` / `grill`, surface evidence and continue.

### Simplify-phase prompt (6b)

```
Run `/simplify` (built-in) on the current branch's diff vs <base>.

Quality cleanup only — reuse / simplification / efficiency / altitude. Do NOT
hunt correctness bugs (that was the review phase). Behavior-preserving edits
only; skip any finding whose fix would change behavior or reach well outside
the reviewed diff, and note the skip.

Mode:
- hitl / grill: surface the findings for user approval BEFORE applying.
- auto: apply + commit via /create-commit (separate `refactor:`/`perf:` commits
  from the fix commits); note skipped findings.

Return: what was cleaned + what was skipped (or "already clean"), commit SHAs.
```
...
All personal skills in `~/.claude/skills` — one curated set, no plugin/marketplace
overhead. The review cascade is self-contained; the rest compose in per phase.
...
- `wayfinder` — multi-session planning for foggy / oversized efforts. Charts the
  route as issue-tracker tickets; each resolved slice hands back into this
  pipeline (see step 1b). one-shot's front-end for anything bigger than one session.
- `grill-with-docs` — every `--mode=grill` checkpoint (brief, PRD, issues, plan,
  findings packet) AND the post-impl ADR-write phase
- `grill-me` — bare-bones grilling; also wayfinder's grilling-ticket engine
- `domain-modeling` — ubiquitous-language / ADR maintenance; wayfinder + grill lean on it
...
- `tdd` — red → green → refactor; the implement phase's TDD contract
- `diagnosing-bugs` — diagnosis loop when implement or review hits a hard bug
...
**External (soft dep — only if installed):**

- `superpowers` — `writing-plans` + `subagent-driven-development`. Any equivalent
  planning + TDD skill works; the orchestrator just needs a plan-file path.

The review cascade (`code-review` → `code-fix`) lives entirely in this set. No
...
Each companion skill (`code-review`, `code-fix`, `create-pr`,
