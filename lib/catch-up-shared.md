...
```bash
bash ~/.claude/lib/catch-up-decide.sh <target-ref>   # main | <pr-base>
```
...
**Mode: lax** — see `~/.claude/lib/commit-push-policy.md`. Catch-up flows are pre-authorized; commit + push without per-turn confirm once lint + tests pass.
